...
// Federation
app.get('/federation/info', (req, res) => res.json({ name: 'OpenChat', version: '1.0.0', channels: channels.size, users: users.size }));
...
function broadcastAll(msg) {
  ...
  PEERS.forEach(peer => {
    fetch(`${peer}/federation/relay`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ from: SERVER_NAME, msg }) }).catch(() => {});
  });
}
...
server.listen(PORT, () => console.log(`OpenChat running on http://localhost:${PORT}`));
